...
| `CreateDevice` / `CreateDeviceWithItem` | Create-ish | `typeIdentifier`, `name`, `deviceItemName` (with-item only), `groupPath` (optional) | `typeIdentifier` is a catalog string like `"System:Device.S71500"` - copy one from an existing similar device's `GetDevices`/`GetDeviceInfo` output rather than guessing. Use `CreateDeviceWithItem` for station-shaped devices (a Device containing a named sub-item, e.g. a CPU station), plain `CreateDevice` for single-item devices (e.g. a drive). |
