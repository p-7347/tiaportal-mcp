...
| `GetNetworkInterfaceInfo` | RO | `path` (device or device item - doesn't need to be exact) | IP/mask/connected-subnet info for a device's network interface. `path` can be a device name or any device item on the way to it (e.g. just `"HMI_1"` or `"PLC_1"`) - it auto-searches nested DeviceItems for the interface, since the nesting depth varies by device type. Check the response's `resolvedPath` to see what was actually used. If more than one interface is found under `path` (e.g. a device with two network cards), the call is rejected with every candidate's full path listed - pass one of those instead of guessing. |
