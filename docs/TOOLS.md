...
> **No GSD install tool**: there is no way to install/import a GSD file through this server -
> confirmed by searching every method name in the installed Openness DLL for anything Gsd-shaped
> (Install/Import/Add/Register), nothing exists. Not just unimplemented - genuinely absent from
> this Openness version's public API.
...
| `GetNetworkInterfaceInfo` | RO | `path` (device or device item - doesn't need to be exact) | IP/mask/connected-subnet/per-port wiring info for a device's network interface. `path` can be a device name or any device item on the way to it (e.g. just `"HMI_1"` or `"PLC_1"`) - it auto-searches nested DeviceItems for the interface, since the nesting depth varies by device type. Check the response's `resolvedPath` to see what was actually used. If more than one interface is found under `path` (e.g. a device with two network cards), the call is rejected with every candidate's full path listed - pass one of those instead of guessing. Each port's `connectedDeviceItemName`/`connectedPortName` reflects the project's own *planned* topology (best-effort, based on an undocumented Openness property - live-verified as working, but can be null even for a wired port if nobody configured it in TIA's Topology view). |
| `CompareNetworkCsv` | RO | `csvPath` | Compares a PRONETA "Device Table (CSV)" export (Network Analysis > Online > Export, with "Include device details" checked - that adds the Port ID/Partner Port/Partner Device columns) against the open project's devices. Read-only - reports matches/mismatches, never writes. Matches by case-insensitive device name only - **MAC-based matching is impossible**, no MAC-address property exists anywhere in this Openness version's public API (confirmed against the shipped XML docs). `matchMethod: null` means no TIA device with that name was found. `ipDiffers` flags a live IP that doesn't match the project's configured IP. Use `SetIpAddress` separately, after a human confirms a reported mismatch, to fix drift. |
| `SetIpAddress` | **Destructive** | `path` (auto-resolving, see `GetNetworkInterfaceInfo`), `ipAddress`, `subnetMask`, `routerAddress` (optional) | Project metadata only - not applied to real hardware until compiled and downloaded. Changing the address can regenerate the PROFINET device-name hash, which can desync from what's downloaded to physical hardware and break the online connection. |
| `ConnectToSubnet` / `DisconnectFromSubnet` | **Destructive** | `path` (auto-resolving), `subnetName` (connect only, from `GetSubnets`) | Same project-metadata-only caveat as `SetIpAddress`. |
| `FindHardwareCatalogEntries` | RO | `filter` (article number/order number/product name fragment) | Wraps `TiaPortal.HardwareCatalog.Find`. **Use this to get the `typeIdentifier` for `CreateDevice`/`CreateDeviceWithItem`** - the catalog's format (`OrderNumber:<article>/<version>`, e.g. `OrderNumber:6ES7 516-3AN01-0AB0/V2.1`) is what the creation APIs actually need. Do NOT copy the `TypeIdentifier` attribute off an existing device (`GetDevices`/`GetDeviceInfo`, format `System:Device.X`) - that's query-only and creation rejects it with "the object cannot be created, it is of the wrong type" (confirmed live, see `CHANGES.md` 2026-09-17). |
| `CreateDevice` / `CreateDeviceWithItem` | Create-ish | `typeIdentifier` (from `FindHardwareCatalogEntries`), `name`, `deviceItemName` (with-item only), `groupPath` (optional) | Use `CreateDeviceWithItem` for anything Siemens models as a station+item pair (drives, CPU stations, HMI panels - live-verified with a CPU 1516-3 PN/DP, which came up with real DP/PROFINET interface DeviceItems). Plain `CreateDevice` only fits catalog entries that are genuinely single-item - it fails "wrong type" for drive/CPU-shaped entries even with a correct catalog `typeIdentifier`. Note: `CreateDeviceWithItem`'s resulting `Device.Name` mirrors `deviceItemName`, not `name` - query the device at `groupPath/deviceItemName`, not `groupPath/name`. |
| `GetDeviceMasterCopies` | RO | - | Lists master copies (saved device/module templates) in the project's own library, for the secondary `CreateFrom(MasterCopy)` creation route. Prefer `CreateDevice`/`CreateDeviceWithItem` with a `FindHardwareCatalogEntries` TypeIdentifier instead - that's verified to work without needing a pre-saved template. |
| `DeleteDevice` | **Destructive**, dry-run by default | `devicePath`, `confirm` (default `false`) | Without `confirm=true` this only previews what would be deleted - nothing changes. TIA's own undo only survives until the next save and hardware config is entangled with physical wiring/GSD matching, so this needs an explicit `confirm=true` on top of whatever confirmation you already got from the user. |
| `CreateDeviceGroup` / `DeleteDeviceGroup` | Create: idempotent-ish / Delete: **Destructive** | `parentGroupPath`/`groupPath`, `name` (create only) | Deleting a group deletes every device inside it. |
